...
const math = require("utils/math_utils");
const cam16_1 = require("./cam16");
const viewing_conditions_1 = require("./viewing_conditions");
/**
...
function getInt(hue, chroma, tone) {
    return getIntInViewingConditions(math.sanitizeDegreesDouble(hue), chroma, math.clampDouble(0.0, 100.0, tone), viewing_conditions_1.ViewingConditions.DEFAULT);
}
/**
 * @param hue CAM16 hue.
...
function findCamByJ(hue, chroma, tone) {
    ...
    while (Math.abs(low - high) > LIGHTNESS_SEARCH_ENDPOINT) {
        mid = low + (high - low) / 2;
        const camBeforeClip = cam16_1.CAM16.fromJch(mid, chroma, hue);
        ...
        if (dL < DL_MAX) {
            const camClipped = cam16_1.CAM16.fromInt(clipped);
            const dE = camClipped.distance(cam16_1.CAM16.fromJch(camClipped.j, camClipped.chroma, hue));
